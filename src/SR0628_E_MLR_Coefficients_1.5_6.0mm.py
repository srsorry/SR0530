"""
SR0628_E: 1.5–6.0° 偏心率上光感受器密度与眼部参数的多元线性回归系数

方法直接复制 SR_FigA_MLR_Split_71eyes.py 中 1.0° 的处理方式：
...
def main():
    ...
    csv_path = os.path.join(OUT_TABLE_DIR, 'SR0628_E_MLR_Coefficients_1.5_6.0deg.csv')
    df_res.to_csv(csv_path, index=False, encoding='utf-8-sig')
    print(f"\nSaved coefficients CSV: {csv_path}")

    # 生成 Markdown 报告
    md = []
    md.append("# SR0628_E 1.5–6.0° 多元线性回归系数表\n\n")
    ...
    for target_name in TARGETS.keys():
        ...
        md.append("| Eccentricity (°) | Predictor | Beta | SE | P value | 95% CI | R² | Adj R² | N |\n")
        md.append("|---------------|-----------|------|----|---------|--------|----|--------|---|\n")
        sub = df_res[df_res['Density_Type'] == target_name].sort_values(['Distance_deg', 'Predictor'])
        for _, row in sub.iterrows():
            ci = f"[{row['CI_Lower_95']:.2f}, {row['CI_Upper_95']:.2f}]"
            p_str = f"{row['P_value']:.4f}" if row['P_value'] >= 0.0001 else "< 0.0001"
            md.append(f"| {row['Distance_deg']:.1f} | {row['Predictor']} | {row['Beta']:.3f} | "
                      f"{row['SE']:.3f} | {p_str} | {ci} | {row['R2']:.3f} | {row['Adj_R2']:.3f} | {int(row['N'])} |\n")
        md.append("\n")

    md.append("---\n\n")
    md.append("*Generated by src/SR0628_E_MLR_Coefficients_1.5_6.0deg.py*\n")

    md_path = os.path.join(OUT_REPORT_DIR, 'SR0628_E_MLR_Coefficients_1.5_6.0deg.md')
