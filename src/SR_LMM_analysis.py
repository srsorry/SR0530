...
def fit_lmm_per_distance(df_long):
    ...
    for dist in distances:
        ...
        try:
            ...
            # OES (Optimal Eccentricity Score) = |beta_AL| * (-log10 P_AL) / ICC
            oes = abs(al_beta) * (-np.log10(al_p + 1e-10)) / (icc + 1e-10)
            ...
            print(f"  {dist:.1f}mm | N={len(sub):3d} eyes/{sub['Subject_ID'].nunique():3d} subj | "
                  ...
                  f"MAPE={mape:.2f}% RMSE={rmse:.1f} | ICC={icc:.3f} | OES={oes:.3f}")
    ...
    return pd.DataFrame(results), qq_data
...
def plot_results(df_results, qq_data):
    ...
    fig_path = os.path.join(OUT_DIR, 'SR0530_LMM_Results.png')
    ...
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    qq_path = os.path.join(OUT_DIR, 'SR0530_LMM_QQPlots.png')
...
def generate_lmm_report(df_results, best_dist_beta, best_dist_score):
    """自动生成 LMM 分析报告"""
    md = []
    md.append("# SR0530 任务一报告：基于双眼数据的 LMM 统计筛选\n")
    md.append("> **目标**：确定 1.0–6.0 mm 偏心率中，角密度受眼部参数影响最显著的点。\n")
    md.append("> **方法**：线性混合效应模型（LMM），Subject_ID 作为随机效应，控制双眼相关性。\n")
    md.append("> **数据**：77 只眼 / 48 名受试者（30 对双眼 + 19 名单眼）。\n\n")
    
    md.append("---\n\n")
    md.append("## 一、LMM 结果汇总\n\n")
    md.append("| 距离 (mm) | 眼数 | 受试者数 | beta_AL | P 值 | R²_边际 | R²_条件 | MAPE(%) | RMSE | ICC | OES |\n")
    md.append("|-----------|------|---------|---------|------|---------|---------|---------|------|-----|-----|\n")
    for _, row in df_results.iterrows():
        sig = "⭐" if row['P_AL'] < 0.05 else ""
        md.append(f"| {row['Distance']:.1f} | {int(row['N_eyes'])} | {int(row['N_subjects'])} | "
                  f"{row['Beta_AL']:.3f} | {row['P_AL']:.4f} {sig} | "
    ...
    md.append(f"**按 OES 最高**：**{best_dist_score:.1f} mm**（综合效应量、显著性与个体差异控制）\n\n")
    
    md.append("## 二、关键指标说明\n\n")
    md.append("- **beta_AL**：AL 的标准化回归系数，表示眼轴每增加 1 个标准差，角密度的变化量。\n")
    md.append("- **R²_边际**：仅由固定效应解释的变异比例。\n")
    md.append("- **R²_条件**：固定效应 + Subject_ID 随机效应共同解释的变异比例。\n")
    md.append("- **MAPE / RMSE**：模型预测误差。\n")
    md.append("- **ICC**（组内相关系数）= σ²_random / (σ²_random + σ²_residual)，反映个体基线差异占总方差的比例。\n")
    md.append("- **OES**（Optimal Eccentricity Score）= |beta_AL| × (-log10(P_AL)) / ICC，综合 AL 效应量、显著性和个体差异控制。\n\n")
    
    md.append("## 三、可视化\n\n")
    md.append("![LMM 综合分析](genData/sum/SR0530_LMM_Results.png)\n\n")
    md.append("![Q-Q 残差图](genData/sum/SR0530_LMM_QQPlots.png)\n\n")
    ...
    md_path = os.path.join(BASE_DIR, 'SR0530_Task1_LMM_Report.md')
...
def main():
    print("=" * 75)
    print("SR0530 Task 1: LMM Analysis with Bilateral Eye Data")
    print("=" * 75)
    ...
    csv_path = os.path.join(OUT_DIR, 'SR0530_LMM_Results.csv')
