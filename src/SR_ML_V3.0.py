...
- 模型：SVM / RF / XGBoost / NN / Lasso / ElasticNet / Ridge
- 标准化回归系数 + Bootstrap 95% CI + MD 报告
"""
...
def main():
    ...
    for dist in distances:
        ...
        for schema_name, feat_short in SCHEMA.items():
            ...
            if n_splits < 2:
                print(f"    Warning: only {n_subjects} subjects at distance {dist}, skipping ML for this distance.")
                continue
    ...
    # 6. Lasso 补充分析：看 ACD 是否被压为 0
    print(f"\nStep 6: Lasso supplementary analysis at {best_dist:.1f} mm...")
    # 使用方案 A2（包含 AL, ACD, Age, Gender）运行 Lasso
...
def generate_md_report(df_ml, df_schema, vif_df, coef_df, lasso_df, best_cfg):
    md = []
    md.append("# SR0530 V3.0 任务二报告：ML 模型优化（重构版）\n\n")
    ...
    md.append("> **数据**：genData/CleanDataRoi_strict/（44 个 ROI，聚合为 11 个距离组）。\n\n")
    ...
    md.append("3. **标准化回归系数替代 SHAP**：鉴于最佳模型多为线性正则化模型，系数及其 Bootstrap CI 比 SHAP 更可靠。\n")
    md.append("4. **Lasso 作为补充**：若 Lasso 将 ACD 压缩为 0，可进一步确立 AL 的核心地位。\n\n")
