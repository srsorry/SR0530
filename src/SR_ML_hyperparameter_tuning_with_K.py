...
# 基线方案（仅用于对比，不重新训练；从已有 q1plus CSV 读取）
BASELINE_MODE_LABEL = 'q1plus'
BASELINE_RESULTS_CSV = os.path.join(OUT_DIR, f'SR0530_HP_Tuning_Results_{BASELINE_MODE_LABEL}.csv')
...
MODE_LABEL = 'q1plus_K' if MIN_QUADRANTS == 1 else 'q4_K'
...
def main():
    ...
    print("Distances: 1.0-6.0 mm | Schemas: +K-augmented only | Models: 7")
    print(f"Baseline results loaded from: {BASELINE_RESULTS_CSV}")
    print(f"Random search iterations per model: {N_ITER}")
    print("=" * 80)

    # Checkpoint 文件：支持断点续跑
    ...
    if os.path.exists(checkpoint_results_csv):
        ...
        completed = set(zip(df_ckpt_results['Data_Group'], df_ckpt_results['Distance_mm']))
        print(f"\nLoaded checkpoint: {len(results)} rows, {len(completed)} (data_group, distance) pairs completed.")
    else:
        results = []
        completed = set()
    ...
    for data_group, data_dir in DATA_DIRS.items():
        ...
        for dist in distances:
            if (data_group, dist) in completed:
            ...
            # 每个 distance 结束后保存 checkpoint
            save_checkpoint()

    # 保存 +K 结果
    df_results_k = pd.DataFrame(results)
    ...
    df_results_k.to_csv(results_csv, index=False, encoding='utf-8-sig')
    ...
    if os.path.exists(BASELINE_RESULTS_CSV):
        ...
        print(f"Loaded baseline results: {len(df_results_base)} rows from {BASELINE_RESULTS_CSV}")
    else:
        print(f"WARNING: baseline results not found at {BASELINE_RESULTS_CSV}; comparison will only show +K results.")
        df_results_base = pd.DataFrame(columns=df_results_k.columns)

    # 合并基线与 +K 结果（schema 名称互不重叠）
    df_results = pd.concat([df_results_base, df_results_k], ignore_index=True)
...
def generate_visualizations(df_results, mode_label='q4'):
    ...
    # 4. 基线 vs +K 对比图
    generate_baseline_vs_K_visualization(df_results, mode_label=mode_label)


# ============================================================
# 基线 vs +K 专用可视化
# ============================================================
def generate_baseline_vs_K_visualization(df_results, mode_label='q1plus_K'):
    """对每个基线方案，比较加入 K 后的最佳 Test R² 变化。"""
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    axes = axes.flatten()

    for idx, (k_schema, base_schema) in enumerate(SCHEMA_BASELINE_MAP.items()):
        ax = axes[idx]
        for data_group in ['strict', 'lenient']:
            df_base = df_results[(df_results['Data_Group'] == data_group) & (df_results['Schema'] == base_schema)]
            df_k = df_results[(df_results['Data_Group'] == data_group) & (df_results['Schema'] == k_schema)]
            if df_base.empty or df_k.empty:
                continue
            best_base = df_base.loc[df_base.groupby('Distance_mm')['test_r2'].idxmax()].sort_values('Distance_mm')
            best_k = df_k.loc[df_k.groupby('Distance_mm')['test_r2'].idxmax()].sort_values('Distance_mm')
            ax.plot(best_base['Distance_mm'], best_base['test_r2'], 'o--', label=f'{data_group} baseline', linewidth=2, markersize=6)
            ax.plot(best_k['Distance_mm'], best_k['test_r2'], 's-', label=f'{data_group} +K', linewidth=2, markersize=6)
        ax.axhline(0, color='gray', linestyle='--', linewidth=0.8)
        ax.set_xlabel('Distance (mm)')
        ax.set_ylabel('Best Test R²')
        ax.set_title(f'{base_schema} vs {k_schema}')
        ax.legend()
        ax.grid(True, alpha=0.3)

    plt.suptitle(f'Baseline vs +Corneal Curvature (K): Best Test R² by Distance ({mode_label})', fontsize=14, fontweight='bold')
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    fig_path = os.path.join(OUT_DIR, f'SR0530_HP_Tuning_Baseline_vs_K_{mode_label}.png')
    plt.savefig(fig_path, dpi=300, bbox_inches='tight')
    plt.savefig(os.path.join(FIG_DIR, os.path.basename(fig_path)), dpi=300, bbox_inches='tight')
    plt.close()
    print(f"  --> Saved: {fig_path}")
...
def generate_report(df_results, mode_label='q4'):
    ...
    md = []
    md.append(f"# SR0530 ML 超参数寻优报告：加入角膜曲率 K（{mode_desc}）\n\n")
    md.append("> **目标**：在原有基线方案（A1/A2/B/C1）基础上加入角膜曲率（K），评估 K 对局部锥细胞密度预测能力的提升。\n\n")
    ...
    # 六、基线 vs +K 对比
    md.append("## 六、基线方案 vs 加入角膜曲率 (K) 的对比\n\n")
    md.append("对每个基线方案及其 +K 版本，按数据组和距离汇总最佳 Test R² 的变化（ΔR² = +K - baseline）。\n\n")
    md.append("| 基线方案 | +K 方案 | 数据组 | 距离 (mm) | 基线 R² | +K R² | ΔR² | 改善 |")
    md.append("|----------|---------|--------|-----------|---------|-------|-----|------|")

    for k_schema, base_schema in SCHEMA_BASELINE_MAP.items():
        df_base_all = df_results[df_results['Schema'] == base_schema]
        df_k_all = df_results[df_results['Schema'] == k_schema]
        for data_group in ['strict', 'lenient']:
            df_base_g = df_base_all[df_base_all['Data_Group'] == data_group]
            df_k_g = df_k_all[df_k_all['Data_Group'] == data_group]
            for dist in sorted(df_results['Distance_mm'].unique()):
                df_base_d = df_base_g[df_base_g['Distance_mm'] == dist]
                df_k_d = df_k_g[df_k_g['Distance_mm'] == dist]
                if df_base_d.empty or df_k_d.empty:
                    continue
                base_r2 = df_base_d['test_r2'].max()
                k_r2 = df_k_d['test_r2'].max()
                delta = k_r2 - base_r2
                improved = '是' if delta > 0.01 else ('持平' if abs(delta) <= 0.01 else '否')
                md.append(f"| {base_schema} | {k_schema} | {data_group} | {dist:.1f} | {base_r2:.3f} | {k_r2:.3f} | {delta:+.3f} | {improved} |")
    md.append("\n")

    # 按方案汇总平均 ΔR²
    md.append("### 按方案汇总的平均 ΔR²\n\n")
    md.append("| 基线方案 | +K 方案 | strict 平均 ΔR² | lenient 平均 ΔR² | 总体平均 ΔR² |")
    md.append("|----------|---------|-----------------|------------------|-------------|")
    summary_rows = []
    for k_schema, base_schema in SCHEMA_BASELINE_MAP.items():
        df_base_all = df_results[df_results['Schema'] == base_schema]
        df_k_all = df_results[df_results['Schema'] == k_schema]
        row = {'base': base_schema, 'k': k_schema}
        deltas = []
        for data_group in ['strict', 'lenient']:
            dg_deltas = []
            df_base_g = df_base_all[df_base_all['Data_Group'] == data_group]
            ...
            for dist in sorted(df_results['Distance_mm'].unique()):
                ...
                base_r2 = df_base_d['test_r2'].max()
                k_r2 = df_k_d['test_r2'].max()
                dg_deltas.append(k_r2 - base_r2)
            avg_dg = np.mean(dg_deltas) if dg_deltas else np.nan
            row[data_group] = avg_dg
            deltas.extend(dg_deltas)
        row['overall'] = np.mean(deltas) if deltas else np.nan
        summary_rows.append(row)
    for row in summary_rows:
        md.append(f"| {row['base']} | {row['k']} | {row['strict']:+.3f} | {row['lenient']:+.3f} | {row['overall']:+.3f} |")
    md.append("\n")
    ...
    md.append(f"### 基线 vs +K 对比 ({mode_label})\n\n")
    md.append(f"![Baseline vs K](FIG/SR0530_HP_Tuning_Baseline_vs_K_{mode_label}.png)\n\n")

    # 九、讨论
    md.append("## 九、讨论\n\n")
    md.append("1. **K 的加入效应**：本报告核心是对比加入角膜曲率（K）后各基线方案 Test R² 的变化。若 ΔR² 普遍接近 0 或为负，说明 K 对预测局部密度没有独立贡献；若 ΔR² 稳定为正，说明 K 携带了 AL/SE/ACD 未捕获的变异。\n")
    md.append("2. **数据组差异**：strict 模式移除了局部 ROI 异常值，数据更干净；lenient 模式保留了更多样本但可能混入异常。\n")
    md.append("3. **最佳参数稳定性**：如果某模型在 strict 和 lenient 下的最佳参数差异很大，提示该模型对异常值敏感。\n")
    md.append("4. **方案选择**：各方案表现因距离和数据组而异，最佳方案需结合 Test R²、Gap 和参数稳定性综合判断，具体见上述结果表。\n")
    md.append("5. **置信区间解释**：R² 与 RMSE 的 95% CI 反映 5-fold CV fold 间变异；R² CI 跨越 0 或 RMSE CI 范围过大，均提示该配置泛化能力不稳定。\n")
    md.append("6. **参数寻优局限**：Random Search 的 n_iter 是计算与精度的折中，关键模型可进一步增加迭代次数。\n\n")
