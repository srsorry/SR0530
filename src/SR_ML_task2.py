...
def main():
    ...
    shap_df.to_csv(os.path.join(OUT_DIR, 'SR0530_Task2_SHAP.csv'), index=False, encoding='utf-8-sig')
...
def generate_md_report(df_ml, df_schema, vif_df, shap_df, best_cfg):
    md = []
    md.append("# SR0530 任务二报告：ML 模型优化（研究方案完整版）\n\n")
    md.append("> **目标**：在锁定 LMM 最佳偏心率后，建立基于常规眼科参数的视锥密度预测模型。\n")
    md.append("> **方法**：双模型策略 + Subject/近视分层 CV + SVM/RF/XGB/NN/Lasso/ElasticNet + SHAP。\n")
    ...
    md.append("2. **小样本下 Lasso/ElasticNet 是重要基线**：线性正则化模型可作为树模型的保守对比。\n")
    md.append("3. **XGBoost/RF 已做复杂度压制**：`max_depth=2–3`，强 L1/L2 正则化，降低过拟合风险。\n")
