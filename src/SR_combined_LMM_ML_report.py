...
def generate_report(lmm_results, ml_results, figure1_file, overlay_file):
    ...
    # 二、ML 结果汇总
    md.append("\n## 二、ML 超参数寻优结果汇总\n\n")
    md.append("| 数据组 | 距离 (mm) | 最佳方案 | 最佳模型 | Test R² (95% CI) | MAPE (%) | N_Eyes |\n")
    md.append("|--------|----------|---------|---------|------------------|----------|--------|\n")
    for dg in ['strict', 'lenient']:
        df_g = ml_results[ml_results['Data_Group'] == dg]
        for dist in sorted(df_g['Distance_mm'].unique()):
            ...
            md.append(f"| {dg} | {best['Distance_mm']:.1f} | {best['Schema']} | {best['Model']} | "
                      f"{best['test_r2']:.3f} [{r2_lo:.3f}, {r2_hi:.3f}] | {best['test_mape']:.2f} | {int(best['N_Eyes'])} |\n")
    ...
    md.append(f"\n2. **ML 最佳预测距离**：\n")
    for dg, idx in best_ml.items():
        row = ml_results.loc[idx]
        r2_lo, r2_hi = compute_t_ci(row['test_r2'], row['test_r2_std'])
        md.append(f"   - {dg.upper()}：{row['Distance_mm']:.1f} mm（Test R² = {row['test_r2']:.3f} "
                  f"[{r2_lo:.3f}, {r2_hi:.3f}]，{row['Model']}）\n")
    ...
    md.append("3. **特征集差异**：LMM 使用完整特征集（含角膜曲率 CC/K），ML 使用精简特征集（AL、ACD、SE、Age、Gender，剔除 K 以避免共线性）。两种分析目的不同，结果需在各自特征集背景下解读。\n")
    md.append("4. **ML 验证价值**：ML 的最佳 R² 曲线可作为 LMM 统计显著性的独立验证；两者共同支持的最优靶点更具可靠性。\n")
    md.append("5. **后续建议**：若 LMM 最敏感点与 ML 最佳点一致，可锁定该距离为最终研究靶点；若不一致，需进一步探索非线性关系或额外特征。\n")
